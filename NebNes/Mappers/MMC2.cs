...
namespace NebNes.Mappers {
    internal class MMC2 : Mapper, IMapper {
        ...
        public MMC2(MOS6502 cpu, Cart cart) : base(cpu, cart) {
        ...
        public byte cpuRead(ushort address) {
            ...
            if(address >= 0x8000 && address <= 0x9FFF) {
                return getPrgPage8K(prgBank)[address - 0x8000];
            }
            if (address >= 0xA000 && address <= 0xBFFF) {
                return getPrgPage8K((prgPages.Length * 2) - 3)[address - 0xA000];
            }
            if (address >= 0xC000 && address <= 0xDFFF) {
                return getPrgPage8K((prgPages.Length * 2) - 2)[address - 0xC000];
            }
            if (address >= 0xE000) {
                return getPrgPage8K((prgPages.Length * 2) - 1)[address - 0xE000];
            }
            return 0;
        }
